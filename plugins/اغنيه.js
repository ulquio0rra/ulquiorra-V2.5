  ...
  try {
    ...
    if (command == 'فيديو') {
      ...
      try {
        ...
        const aa_2 = await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `*『🐉┇𝕌𝕝𝕢𝕦𝕚𝕠𝕣𝕣𝕒 𝔹𝕠𝕥』\n انا لا اتحمل ذنب اغانيك او ما تشاهده*`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
        if (!aa_2) {
        throw new Error('*[❗] El primero metodo fallo, intentando otro...*');
       }
      } catch {
        ...
        const json = await res.json();
        await conn.sendFile(m.chat, json.result.video, 'error.mp4', `*『🐉┇𝕌𝕝𝕢𝕦𝕚𝕠𝕣𝕣𝕒 𝔹𝕠𝕥』\n انا لا اتحمل ذنوب اغانيك او ما تشاهده*`, m);
      }
    }
  } catch {
